...
public class SignUpService {
	...
	public ResponseEntity<?> signUpUser(SignUpRequest request) {
		...
		if (request.getQuestion1().isBlank()) {

			return ResponseEntity.badRequest().body(new SignUpResponse("Enter security question for recovery! "));
		}
		if (request.getQuestion2().isBlank()) {
		...
		SecurityQuestion question1 =	securityQuestionsRepository.findByquestionName(request.getQuestion1()).get();
		
		SecurityQuestion question2 =	securityQuestionsRepository.findByquestionName(request.getQuestion2()).get();

		question2.setStatus(true);
		question1.setStatus(true);
		securityQuestionsRepository.save(question1);
		securityQuestionsRepository.save(question2);
		userRepository.save(userInfo);
		UserInfo userInfo2 = userRepository.findByUsername(userInfo.getUsername()).get();
		UserSecurityQuestion userSecurityQuestion = new UserSecurityQuestion(userInfo2.getId(), question1.getId() ,request.getAnswer1());
		UserSecurityQuestion userSecurityQuestion2 = new UserSecurityQuestion(userInfo2.getId(), question2.getId() ,request.getAnswer2());

		userSecurityRepository.save(userSecurityQuestion);
		userSecurityRepository.save(userSecurityQuestion2);
